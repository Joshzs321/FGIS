...
var DrawHelper = (function () {
    ...
    function _(cesiumWidget) {
        ...
        this._surfaces = [];
        // 该方法写在_.prototype上，这样才能调用：因为this会绑定到“实例化”的对象上，该对象的prototype和_.prototype是一个对象
    ...
    // 一次性给所有实体绑定监听事件，具体的响应函数在实体构建时绑定到实体对应的绑定事件中
    _.prototype.initialiseHandlers = function () {
        ...
        var mouseOutObject;
        // 鼠标移动事件，实体上可能绑定了mouseMove和mouseOut事件
        handler.setInputAction(
            function (movement) {
                if (_self._handlersMuted == true) return;
    ...
    _.prototype.registerEditableShape = function (surface) {
        ...
        setListener(surface, 'mouseMove', function (position) {
            ...
            if (!surface._editMode) {
                _self._tooltip.showAt(position, "点击编辑要素");
            }
        });
    ...
    material.uniforms.color = new Cesium.Color(0.0, 0.0, 1.0, 0.3);
    ...
    _.ZonePrimitive = (function () {
        ...
        function _(options) {
            ...
            if (options.extrudedHeight) {
                this.setExtrudedHeight(options.extrudedHeight);
            }
        ...
        _.prototype.getExtrudedHeight = function () {
            return this.getAttribute('extrudedHeight');
        };
        _.prototype.setExtrudedHeight = function (extrudedHeight) {
            this.setAttribute('extrudedHeight', extrudedHeight);
        };
        _.prototype.getGeometry = function () {
        ...
        _.prototype.getOutlineGeometry = function () {
            return new Cesium.CircleOutlineGeometry({
                center: this.getCenter(),
                radius: this.getRadius(),
                extrudedHeight: this.getExtrudedHeight()
            });
        }
    ...
    _.HemispherePrimitive = (function () {
    ...
    _.EllipsePrimitive = (function () {
        function _(options) {
            ...
            // 这个方式应该是ChangeablePrimitive原型对象上的
            // fucntion本来有一个自己原型对象，但是为了将不同实体构造函数的一些通用的方法统一到一起（ChangeablePrimitive），
            // 就把各种实体构造函数的prototype修改为了ChangeablePrimitive
            this.initialiseOptions(options);
    ...
    _.prototype.startDrawingMarker = function (options) {
        ...
        mouseHandler.setInputAction(function (movement) {
            ...
            if (position != null) {
                ...
                if (cartesian) {
                    tooltip.showAt(position, "<p>点击添加点. 当前位置: </p>" + getDisplayLatLngString(ellipsoid.cartesianToCartographic(cartesian)));
                } else {
                    tooltip.showAt(position, "<p>点击添加点.</p>");
                }
            }
        }, Cesium.ScreenSpaceEventType.MOUSE_MOVE);
    ...
    _.prototype.startDrawingPolyshape = function (isPolygon, options) {
        ...
        mouseHandler.setInputAction(function (movement) {
            ...
            if (position != null) {
                ...
                if (positions.length == 0) {
                    if (cartesian) {
                        tooltip.showAt(position, "<p>点击添加第一个点. 当前位置: </p>" + getDisplayLatLngString(ellipsoid.cartesianToCartographic(cartesian)));
                    }
                } else {
                    //var cartesian = scene.camera.pickEllipsoid(position, ellipsoid);
                    if (cartesian) {
                        ...
                        // show tooltip
                        tooltip.showAt(position, "<p>点击添加新点 (" + positions.length + ")" + (positions.length > minPoints ? "或双击完成绘制" : "") + "<p>当前位置: </p>" + getDisplayLatLngString(ellipsoid.cartesianToCartographic(cartesian)));
    ...
    _.prototype.startDrawingExtent = function (options) {
        ...
        function updateExtent(value) {
            if (extent == null) {
                extent = new Cesium.Rectangle();//RectanglePrimitive();
        ...
        mouseHandler.setInputAction(function (movement) {
            ...
            if (position != null) {
                if (extent == null) {
                    tooltip.showAt(position, "<p>点击添加矩形</p>");
                } else {
                    var cartesian = scene.camera.pickEllipsoid(position, ellipsoid);
                    if (cartesian) {
                        var value = getExtent(firstPoint, ellipsoid.cartesianToCartographic(cartesian));
                        updateExtent(value);
                        tooltip.showAt(position, "<p>拖拽改变矩形范围</p><p>再次点击完成绘制</p>");
                    }
                }
            }
        }, Cesium.ScreenSpaceEventType.MOUSE_MOVE);
    ...
    _.prototype.startDrawingZone = function (options) {
            ...
            if (movement.position != null) {
                ...
                if (cartesian) {
                    if (zone == null) {
                        ...
                        //通过回调函数实现绘制后的成图
                        if (typeof options.callback == 'function') {
                            options.callback(zone.getCenter(), zone.getRadius(), zone.getExtrudedHeight());
                        }
                        _self.stopDrawing();
                    }
                }
        ...
        mouseHandler.setInputAction(function (movement) {
            ...
            if (position != null) {
                if (zone == null) {
                    tooltip.showAt(position, "<p>点击开始画禁飞区</p>");
                } else {
                    var cartesian = scene.camera.pickEllipsoid(position, ellipsoid);
                    if (cartesian) {
                        zone.setRadius(Cesium.Cartesian3.distance(zone.getCenter(), cartesian));
                        markers.updateBillboardsPositions(cartesian);
                        tooltip.showAt(position, "<p>移动鼠标改变半径</p><p>再次点击完成绘制</p>");
                    }
                }
                ...
                if (editMode) {
                    ...
                    if (this._markers == null) {
                        ...
                        var handleMarkerChanges = {
                            ...
                            tooltip: function () {
                                return "Drag to change the excentricity and radius拖拽改变偏心率和半径";
                            }
                        };
    ...
    // 绘制工具条对象
    // 为什么要添加在构造函数上，作为局部函数也是没有问题的
    _.DrawHelperWidget = (function () {

        // constructor
        // 这是生成的另一个对象，如果要使用drawHelper的内容，需要传进来
        function _(drawHelper, options) {
            ...
            addIcon('file', options.fileIcon, '加载图形文件', function(){
                $("#shapeFilePath").click()
            })

            addIcon('marker', options.markerIcon, '点击开始画点', function () {
            ...
            addIcon('polyline', options.polylineIcon, '点击开始画线', function () {
            ...
            addIcon('polygon', options.polygonIcon, '点击开始画多边形', function () {
            ...
            addIcon('circle', options.circleIcon, '点击开始画圆', function () {
            ...
            // 绘制禁飞区
            addIcon('zone', options.zoneIcon, '点击开始画禁飞区', function () {
                drawHelper.startDrawingZone({
                    callback: function (center, radius, extrudedHeight) {
                        _self.executeListeners({ name: 'zoneCreated', center: center, radius: radius, extrudedHeight: extrudedHeight });
                    }
                });
            })
            ...
            toolbar.appendChild(div);
            addIcon('clear', options.clearIcon, '清除所有绘制的要素', function () {
    ...
    function getDisplayLatLngString(cartographic, precision) {
        ...
        var longtitude = Cesium.Math.toDegrees(cartographic.longitude).toFixed(precision || 5);
        var latitude = Cesium.Math.toDegrees(cartographic.latitude).toFixed(precision || 5);
