...
router.get('/', function (req, res, next) {
 // res.render('login', {title: 'login'});   修改的地方
    ...
    connection.query($sql, [username], function (err, result) {
      ...
      if (resultJson.length === 0) {
        ...
        connection.query($sql1, [username], function (err, result) {
          ...
          // 判断是否为管理员
          var $sql3 = "select authority from users where username=?";
          connection.query($sql3, [username], function (err, result) {
            var temp1 =result[0].authority; 

            if (temp1 == 'administrator') {
              result = {
              code: 500,
              msg: '管理员登录'
            };
